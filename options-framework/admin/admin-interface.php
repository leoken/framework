...
function of_load_only() {
	...
	// Google Maps Picker
	wp_enqueue_script('googlemapsapi', 'http://www.google.com/jsapi?key=ABQIAAAAviYjaR8FeZKoJ0iOyuOw3BT2yXp_ZAY8_ufC3CFXhHIE1NvwkxSuUsx66fC9iWRGJmqDPp28yTYt1w');
}
...
<script type="text/javascript" language="javascript">
		...
		function updateLatLonFields(lat, lon) {
			oFormObject.elements["hidden_tf_googlemapspicker_lat"].value = lat;
			oFormObject.elements["hidden_tf_googlemapspicker_lon"].value = lon;
			}
		...
		function showMap() {
			...
			google.maps.Event.addListener(gmap, "zoomend", function(oldZoom, newZoom) {
				...
				oFormObject.elements["hidden_tf_googlemapspicker_zoom"].value = newZoom;
			});
			...
			gmap.setCenter(
				new google.maps.LatLng(<?php echo $latitude; ?>, <?php echo $longitude; ?>), <?php echo $zoom; ?>
			);
...
function optionsframework_machine($options) {
	...
	foreach ($options as $value) {
		...
		case "googlemapspicker":
			...
			$output .= '<div id="bd"> ';
			$output .= '<div id="gmap" style="width:595px;height:400px"></div> ';
			$output .= '<div id="crosshair" style="width:19px;height:19px;position: absolute;
    top: 192px;
    left: 50%;
    margin-left: -8px;
    display: block;
    background: url(' . OF_DIRECTORY . '/admin/js/crosshair.gif);
